<template>
  <HomeLayout>
    <section class="min-h-screen bg-shark-950 pb-28 text-white">
      <div class="mx-auto grid max-w-5xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[14rem_minmax(0,42rem)] lg:justify-center">
        ...
        <main class="mx-auto min-w-0 w-full max-w-2xl space-y-4">
            ...
            <div class="mt-0 grid gap-4" :class="{ 'mt-4': !isLoggedIn }">
              ...
              <div class="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                ...
                >
                  ...
                  <option v-if="albums.length === 0" value="" disabled>No albums available</option>
          ...
          <section class="space-y-4">
            ...
            <article
              v-for="post in posts"
              :key="post.id"
              class="rounded-lg border border-white/10 bg-white/[0.035] p-4 shadow-lg shadow-black/10 sm:p-5"
            >
              <div class="flex gap-3">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div v-if="canManagePost(post)" class="mt-2 flex gap-2">
                    ...
                    <button type="button" class="owner-button danger" :disabled="post.isBusy" @click="deletePost(post)">Delete</button>
                  </div>
                  ...
                  <div v-if="post.showComments" class="mt-4 space-y-3 rounded border border-white/10 bg-black/25 p-3">
                    <div v-for="comment in post.comments" :key="comment.id" class="flex gap-3">
                      <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-white/10 text-xs font-black text-gray-300">
                        {{ initials(comment.user?.name || comment.user?.email || 'User') }}
                      </div>
                      <div class="min-w-0">
                        <p class="text-xs font-black text-gray-500">{{ comment.user?.name || comment.user?.email || 'Retro Listener' }}</p>
...
const loadPosts = async () => {
  ...
  try {
    ...
    posts.value = (response.data.data || []).map(normalizePost);
  } catch (err) {
...
const loadProducts = async () => {
  try {
    ...
    if (products.length) {
      albums.value = products.map((product) => ({
        id: product.id,
        title: product.title,
...
const deletePost = async (post) => {
  if (post.isBusy || !window.confirm('Delete this post?')) return;
  post.isBusy = true;
